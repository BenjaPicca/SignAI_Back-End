...
const getTotal = async (req, res) => {
    ...
    if (!mail) return res.status(400).json({ message: "Falta el mail" });
    try {
...
const getByJuego = async (req, res) => {
    ...
    if (!mail) return res.status(400).json({ message: "Falta el mail" });
    try {
        const rows = await Puntaje.getByJuegoMail(mail);
        return res.status(200).json({ mail, juegos: rows });
    } catch (err) {
        console.log(err);
        return res.status(500).json({ message: "Error al obtener puntaje por juego" });
    }
};

const addScore = async (req, res) => {
    const { mail, juego, puntos } = req.body;
...
export default { getTotal, getSemanal, getByJuego, addScore };
